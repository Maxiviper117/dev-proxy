...
/**
 * Return a Vite allowedHosts warning for project-local open commands.
 *
 * This intentionally supports static config shapes only. Dynamic Vite config
 * can still be valid, so DevProxy warns instead of blocking the browser open.
 */
async function getViteAllowedHostsWarning(
  cwd: string,
  domain: string,
