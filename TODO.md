...
- [x] Add an integration test mode that uses temporary Caddy/hosts paths and never touches the real system.
- [x] Add a `devproxy status` command that reports Caddy running state, registered services, and upstream reachability.
- [x] Add a `devproxy trust` helper that checks Caddy trust state and prints the exact elevated `caddy trust` command when needed.
